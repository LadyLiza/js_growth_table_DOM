...
appendColumnButton.onclick = ()=>{
    if (table.rows[0].cells.length < maxColumns) for(let i = 0; i < table.rows.length; i++)table.rows[i].insertCell();
    updateButtons();
};
removeColumnButton.onclick = ()=>{
    if (table.rows[0].cells.length > minColumns) for(let i = 0; i < table.rows.length; i++)table.rows[i].deleteCell(-1);
    updateButtons();
};
updateButtons();
